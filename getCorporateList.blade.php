...
<div class="col-md-12">
  ...
  <form method="post" action="{{URL::to('admin/filterCorporate')}}" >
    <div class="row">
      <div class="form-group">
        <div class="col-md-3">
          <select class="form-control" name="corporate_status">
            @if($corporate_status==0)
              <option value="0" selected="selected">Created</option>
            @else
              <option value="0">Created</option>
            @endif
            @if($corporate_status==1)
              ...
              @if($corporate_status==0)
                ...
                <td>
                  @if($appUser->id  !== $row->first_approval)
                    <a class="btn btn-info" href="{{ route('corporateStatusApproved', ['id' => $row->id , 'account_actived'=>1])}}">Approve</a>
                    <a class="Reject btn btn-info" id="{{$row['id']}}" data="{{$row->id}}">Reject</a>
                    <a class="btn btn-info" href="{{ URL::to('getCert/'.$row->company_certificate_url)}}" target="_blank"  style="text-align:center;display:inline-block;margin:auto 0;">View Document</a>
                    <!--20181025 Change by SHC 20180816_CP_002 Start -->                   
                    @if(!is_null($row->company_certificate_url2))
                      <a class="btn btn-info" href="{{ URL::to('getCert/'.$row->company_certificate_url2)}}" target="_blank"  style="text-align:center;display:inline-block;margin:auto 0;">View Document2</a>  
                    @endif         
                  ...
                  @elseif($appUser->id  == $row->first_approval)
                    {{--  <a><button class="Reject" id="{{$row['id']}}" data="{{$row->id}}" style="font-size: 20px;float: right;margin-top: 0px;background: #FFFFFF;clear: right;" >Reject</button> </a> --}}
                  @endif
                </td>
              @endif
              @if($corporate_status==1)
                ...
                <div class="form-group clearfix">
                  ...
                  <div class="col-md-8">
                    <select class="form-control" name="reasonType" id="reasonType">
                      <option value="1">Invalid Corporate</option>
...
{{$corporate->appends(Request::except('page'))->links()}}
@stop
    ...
    $(document).ready(function()
    {
      ...
      $(document).on('submit', '#rejectForm', function(e)
      {
        ...
        if(text != "")
        {
          ...
          window.location.href = '{{URL::to('admin/corporateStatusReject')}}/'+id+'/'+text+'/'+rejectType;
        }
        else
        {
          var rejectType = $("#reasonType").val();
